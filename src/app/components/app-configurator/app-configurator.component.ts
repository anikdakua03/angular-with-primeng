...
export class AppConfiguratorComponent {
  ...
  configService: ThemeService = inject(ThemeService);
  ...
  primaryColors = computed(() => {
    ...
    const colors = [
      'emerald', 'green', 'lime', 'orange', 'amber', 'yellow', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose'
    ];
  ...
  getPresetExt() {
    ...
    if (color !== undefined && color.name === 'noir') {
      return {
        semantic: {
          primary: {
            50: '{surface.50}',
      ...
      if (this.configService.appState().preset === 'Nora') {
        return {
          semantic: {
            primary: color !== undefined && color.palette,
            colorScheme: {
              light: {
                primary: {
                  color: '{primary.600}',
